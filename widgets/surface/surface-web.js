...
define(['yarnball/src/node', 'yarnball/src/batch', 'yarnball/src/map', 'yarnball/src/str', 'yarnball/src/number'], function(Node, Batch, Map_, Str, Number) {
  ...
  SurfaceWeb.prototype.getWidgets = function() {
    var self = this;
    return self._web.query(self._base, Owns, null).then(function(ownedObjects) {
      var filterPromises = ownedObjects.map(function(ownedObject) {
        return self._web.hasLink(ownedObject, Is, Widget);
      });
      return Promise.all(filterPromises).then(function(filter) {
        return ownedObjects.filter(function(ownedObject, index) {
          return filter[index];
        });
      });
    });
...
//       throw 'Cannot add widget to surface-web, unknown widget type: ' + widget.widgetType;
  ...
  SurfaceWeb.prototype.initializeSurface = function(surface) {
    ...
    self.getNodeWidgets().then(function(NodeWidgets) {
    ...
    this.getConnectors().then(function(Connectors) {
